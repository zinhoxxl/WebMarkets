CREATE DEFINER=`root`@`localhost` TRIGGER `member_AFTER_UPDATE` AFTER UPDATE ON `member` FOR EACH ROW BEGIN
insert into history_memberUpdate(seq,id, oldName,  oldGender, oldBirth, oldMail, oldPhone, oldZipcode,
